...
bool LocalAgentToolSession::start(AgentMcpBindings bindings,
    ...
    if (bindings_.document != bindings.document || bindings_.task != bindings.task) {
        autosave_task_ = 0;
...
{"type":"function","name":"pixelforge_pack","description":"Create and work on an arbitrary set of sprite canvases inside one task. Use create with canvases='name,group,width,height,frame|...' for variants or animation frames. add appends new canvases using the same specification without replacing existing artwork; optional source seeds every new same-size frame from one existing canvas. Duplicate names reject the whole add. Added frames are structural additions and are not removed by pixel history undo. program accepts normal PixelProgram syntax plus CANVAS name directives and can update many canvases in one call. list and analyze return pages (offset,limit default64 max256); analyze caches bounds, centroid, colors, pixel hashes and adjacent/loop deltas without images. view returns a bounded image page (default32 max128) with next_offset; export keeps full resolution. save retries persistence without editing. clone/copy reuse frames; history undoes/redoes an entire multi-canvas artistic pass; inspect reads exact pixels; export saves one canvas, a group/all PNGs, sheet/strip PNG, or animation GIF.","inputSchema":{"type":"object","properties":{"action":{"type":"string","enum":["create","add","list","analyze","save","program","edit","clone","copy","history","view","inspect","export"]},"task_id":{"type":"integer"},"pixel_offset":{"type":"integer","minimum":0},"pixel_limit":{"type":"integer","minimum":1,"maximum":4096},"summary":{"type":"boolean","description":"list/analyze: one row per group instead of per frame; use first for large workspaces."},"offset":{"type":"integer","minimum":0},"limit":{"type":"integer","minimum":1,"maximum":256},"known_observation":{"type":"string"},"resend_image":{"type":"boolean"},"canvases":{"type":"string"},"canvas":{"type":"string"},"group":{"type":"string"},"program":{"type":"string"},"patch":{"type":"string"},"source":{"type":"string"},"dest":{"type":"string"},"direction":{"type":"string","enum":["undo","redo"]},"mode":{"type":"string","enum":["canvas","sheet","strip","animation"]},"scope":{"type":"string","enum":["canvas","group","all","sheet","strip","animation"]},"render_scale":{"type":"integer","minimum":1,"maximum":16},"scale":{"type":"integer","minimum":1,"maximum":16},"columns":{"type":"integer","minimum":1},"fps":{"type":"integer","minimum":1,"maximum":60},"x":{"type":"integer"},"y":{"type":"integer"},"width":{"type":"integer"},"height":{"type":"integer"},"sx":{"type":"integer"},"sy":{"type":"integer"},"dx":{"type":"integer"},"dy":{"type":"integer"},"path":{"type":"string"}},"required":["action","task_id"]}},
...
{"type":"function","name":"pixelforge_history","description":"Undo or redo a canvas revision.","inputSchema":{"type":"object","properties":{"action":{"type":"string","enum":["undo","redo"]},"task_id":{"type":"integer"},"expected_revision":{"type":"integer"}},"required":["action","task_id"]}},
{"type":"function","name":"pixelforge_io","description":"Export the current canvas to a native PNG.","inputSchema":{"type":"object","properties":{"action":{"type":"string","enum":["export"]},"task_id":{"type":"integer"},"expected_revision":{"type":"integer"},"path":{"type":"string"}},"required":["action","task_id","path"]}},
{"type":"function","name":"pixelforge_record","description":"Start, query, or stop local session recording.","inputSchema":{"type":"object","properties":{"action":{"type":"string","enum":["start","status","stop"]},"task_id":{"type":"integer"},"fps":{"type":"integer","minimum":1,"maximum":60}},"required":["action","task_id"]}}
])JSON";
}
